...
# Reject H0 if p <= alpha, p<=0.1
t.test(decay$strength[decay$weeks==2], 
       decay$strength[decay$weeks==16],
       conf.level=0.9)
...
minLength <- min(length(decay$strength[decay$weeks==2]), length(decay$strength[decay$weeks==16]))

margin.Off.error <- qt(0.05 , df = 4, lower.tail = F ) * sqrt( sd(decay$strength[decay$weeks==2])^2 / minLength  +  sd(decay$strength[decay$weeks==16])^2 / minLength    )
