#ROC Curves for logistic regression and random forests#
...
data$prob <-predict(m2,data, type="response")

roc.info <-roc(data$event ~ data$prob, legacy.axes=T)
roc(data$event,data$prob,direction="<")
...
roc.df <-data.frame(
  sensitivity=roc.info$sensitivities * 100,
...
roc.df[roc.df$sensitivity > 40 & roc.df$specificity < 80, ]
...
roc(data$event ~ data$prob, plot=TRUE, legacy.axes=T, 
    percent=T, 
    xlab="False Positive (%)", 
    ylab="True Positive (%)", col="blue", lwd=4, 
    print.auc=T, print.auc.x=45, 
...
rf.model <- randomForest(formula = factor(event) ~ age + chol + sex , data=data, importance = TRUE)
rf.model

# Visualize the first Tree
tree_func(rf.model,1)
