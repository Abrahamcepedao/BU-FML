### Data filtering, aggregation, and t-tests ###
...
# Make sure you have the correct project setup as we discussed in class
# Otherwise you will need to change the path below
...
# check the length of each of the variables that you have created 
...
#Summary statistics for the heights of sons / daughters
summary(data$child[data$chl=="Son"])
summary(data$child[data$chl=="Daughter"])
...
#Dplyr version of the above for grouping and analysis

#Dplyr is a modern package for aggregation/grouping/summarization/etc.
...
data %>% 
  group_by(chl) %>% 
  summarise(mean_child = mean(child))

#Get smallest height by group
data %>% group_by(chl) %>% slice(which.min(child))
...
# Another way to summarise the data (dplyr)
require(dplyr)

tmp = data %>% group_by(chl) %>% summarise(m = mean(child),high = t.test(child)$conf.int[2],low=t.test(child)$conf.int[1])
tmp$ci = tmp$high-tmp$m


ggplot(tmp,aes(x=chl,y=m,fill=chl)) + 
  geom_bar(position="dodge",stat="identity") +
  geom_errorbar(aes(ymin=m-ci,ymax=m+ci),width=0.2) + 
  theme_bw() + 
  xlab("") + 
  ylab("Average Height") + 
  guides(fill=guide_legend(title="Sex of Child"))
